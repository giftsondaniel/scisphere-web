...
def create_async_export(xform, export_type, query, force_xlsx, options=None):
    ...
    if export_type in [Export.XLS_EXPORT, Export.GDOC_EXPORT,
        ...
        if export_type in [Export.XLS_EXPORT, Export.GDOC_EXPORT]:
            ...
            result = create_xls_export(username, id_string, export_id, query)
        elif export_type == Export.CSV_EXPORT:
            #result = create_csv_export.apply_async((), arguments, countdown=10)
	    result = create_csv_export(username, id_string, export_id, query)
        elif export_type == Export.CSV_ZIP_EXPORT:
            #result = create_csv_zip_export.apply_async((), arguments, countdown=10)
            result = create_csv_zip_export(username, id_string, export_id, query)
        else:
            raise Export.ExportTypeError
    elif export_type == Export.ZIP_EXPORT:
        # start async export
        #result = create_zip_export.apply_async((), arguments, countdown=10)
        result = create_zip_export(username, id_string, export_id, query)
    elif export_type == Export.KML_EXPORT:
        # start async export
        #result = create_kml_export.apply_async((), arguments, countdown=10)
        result = create_kml_export(username, id_string, export_id, query)
    else:
        raise Export.ExportTypeError
    if result and job:
        ...
        print(result)
        export = Export.objects.get(id=export_id)
        export.task_id = job.id #result.task_id
        export.save()
