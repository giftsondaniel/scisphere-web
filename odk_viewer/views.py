...
def create_export(request, username, id_string, export_type):
	...
	#create export object
        #export = _create_export(xform, export_type) # create export object
        #time.sleep(1)
	
	queue = django_rq.get_queue('low')
	queue.enqueue(create_async_export, xform, export_type, query, force_xlsx, options)
        time.sleep(1)
...
def export_list(request, username, id_string, export_type):
    ...
    if should_create_new_export(xform, export_type):
	    ...
	    queue = django_rq.get_queue('low')
	    queue.enqueue(create_async_export, xform, export_type, query=None, force_xlsx=True)
	    time.sleep(2)
            #create_async_export(xform, export_type, query=None, force_xlsx=True)
        except Export.ExportTypeError:
            return HttpResponseBadRequest(
                _("%s is not a valid export type" % export_type))
...
def data_view1(request, id_string):
    #owner = get_object_or_404(User, username=username)
    #xform = get_object_or_404(XForm, id_string=id_string, user=owner)
    ...
    audit_log(
        Actions.FORM_DATA_VIEWED, request.user,
        _("Requested data view for '%(id_string)s'.") %
        {
            'id_string': xform.id_string,
        }, audit, request)
