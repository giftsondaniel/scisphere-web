...
class ParsedData(models.Model):
    USERFORM_ID = u'_xform_id_string'
    STATUS = u'_status'
    DEFAULT_LIMIT = 30000
    ...
    def query_mongo_data(cls, username, id_string, query, fields, sort, start=0,
                    limit=DEFAULT_LIMIT, count=False, hide_deleted=True):
	print("query 1111111111111")
	#print(query)
        ...
        query[cls.USERFORM_ID] = u'%s' % (id_string)
        #if hide_deleted:
	...
	listobj = []
#        for objval in cursor:
#	   objvalid = objval["_id"]
#	   #print dir(recordid)
#	   objval["_id"] = objvalid.__str__()
#	   listobj.append(objval)

	#response_text = simplejson.dumps(listobj)
	#print(response_text)
        #cursor = xform_instances.find({"_xform_id_string":"TN_Population_2001"})
        if count:
	    print("ccccccccc")
    ...
    @classmethod
    def getCount(cls,id_string):
	#print(id_string)
	query = {}
        query['_xform_id_string'] = u'%s' % (id_string)
