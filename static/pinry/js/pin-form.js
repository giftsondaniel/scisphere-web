...
$(window).load(function() {
    ...
    function createPinPreviewFromForm() {
        ...
        setTimeout(function() {
            if (preview.find('.pin').height() > 250) {  // 305
                $('#pin-form .pinry-modal-body').animate({
    ...
    function createPinForm(editPinId) {
        ...
        $('#pin-form-image-upload').fineUploader({
            request: {
                endpoint: '/app/pins/create-image/',
                paramsInBody: true,
                multiple: false,
                validation: {
        ...
        $('#pin-form-submit').click(function(e) {
            ...
            if (editedPin) {
                var apiUrl = '/app/api/v1/pin/'+editedPin.id+'/?format=json';
		console.log(apiUrl);
                var data = {
                    description: $('#pin-form-description').val(),
                    tags: cleanTags($('#pin-form-tags').val())
                ...
                var data = {
                    submitter: '/app/api/v1/user/'+currentUser.id+'/',
                    description: $('#pin-form-description').val(),
                    tags: cleanTags($('#pin-form-tags').val())
                };
                if (uploadedImage) data.image = '/app/api/v1/image/'+uploadedImage+'/';
                ...
                promise.error(function(error) {
                    console.log(error);
                    message('This is Invalied URL.', 'alert alert-error');
                });
            }
        });
