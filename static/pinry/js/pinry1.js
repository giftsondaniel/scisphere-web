...
$(window).load(function() {
	...
	if(pinTotalPageLength >= offset) loadPins();
    }
...
function pinPagination(pinTotalPageLength){
	...
	var pin_row_limit = 4;
	pinTotalPage = Math.ceil(pinTotalPageLength / pin_row_limit);
...
function pinPaginationGoTo(){
	...
	$pin_table_rows = $('.pins-load div.pin');
	var pin_row_limit = 4;
	pinTotalPage = Math.ceil($pin_table_rows.length / pin_row_limit);
	$('#pg1').jqPagination('option', 'max_page', pinTotalPage);
