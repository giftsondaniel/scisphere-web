  ...
  function chart(selection) {
    selection.each(function(data) {
     ...
     /* if(dtlng > 500){
      ...
      var gEnter = svg.enter().append("svg").attr('id','bar_svg').append("g");
     ...
     // Update the bars.
      var bar = svg.select(".bars").selectAll(".bar_svg").data(data);
...
//      bar .attr("class", function(d, i) { return d[1] < 0 ? "bar" : "bar"; })
	  ...
	  .on('mouseover', function(d) {
			...
			if(isNaN(d[1])){
			 	d3.select('#spacialcontent').html( did[0].toTitleCase() + ' <span class="value">(' +"Inf"+ ')</span> &nbsp;' + '<span style="color:' + + '">&#9608;</span>')
				.style('opacity', 1) 			
			}else{
				var value = '';
				if(isInteger( d[1])){
				...
				d3.select('#spacialcontent').html( did[0].toTitleCase() + ' <span class="value">(' + value + ')</span> &nbsp;' + '<span style="color:' + + '">&#9608;</span>')
				.style('opacity', 1)} })
