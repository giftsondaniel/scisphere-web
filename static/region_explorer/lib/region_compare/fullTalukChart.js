function level_3_LevelChart( fld, yr,chartWidth,chartHeight,level1Id){
			...
			if((QrMaxValue < maxvall) && (QrMaxValue != 0) ){ 
				...
				svg.selectAll("rect.lower")
				   ...
				   .on('mouseover', function(d) {
						...
						d3.select('#spacialcontent_'+level1Id).html( d.name_3 + ' <span class="value">(' + d.value + ')</span> &nbsp;' + '<span style="color:' + + '">&#9608;</span>').style('top', mouse[1] + 'px').style('left', (((mouse[0]) ) + 'px')) 
						.style('opacity', 1) })
				...
				svg.append("g").attr("transform", "translate(240,0)")
						...
						.style({"font": "Roboto, sans-serif",'font-size':'10px',"fill":"#8A8A8A"});
