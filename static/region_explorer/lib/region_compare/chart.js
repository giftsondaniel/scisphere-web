...
function chart(fld, yr,chartWidth,chartHeight,level1Id){
	...
	var data = datas.sort(function(a,b) {
		...
		d3.select("#bar_"+level1Id)
		    .datum(barData)
		    .call(columnChart(columnBarChartWidth,level1Id)
		    .width(chartWidth)
		    .height(450)
			...
			if((QrMaxValue < maxvall) && (QrMaxValue != 0) ){
				...
				var svg = d3.select("#bar_"+level1Id).append("svg").attr("width",chartWidth).attr("height", 450).attr('id','bar1').append("g").attr("id", "barChart")
				...
				svg.selectAll("rect.lower")
				   ...
				   .on('mouseover', function(d) {
						...
						d3.select('#spacialcontent_'+level1Id).html( d.name_2 + ' <span class="value">(' + d.value + ')</span> &nbsp;' + '<span style="color:' + + '">&#9608;</span>').style('top', mouse[1] + 'px').style('left', (((mouse[0]) ) + 'px')) 
						.style('opacity', 1) })
				   .on('mouseout', function(d) {
					...
					snapLevel = 3;
					snapURL = "scisphere/places/"+level1_Id+"/"+d.id_2+"?level="+snapLevel+"&level0="+level0_Id+"&year="+yearId;	
					canvasImage = "chart";
	...
	/*if(datas.length == 0){
		var svg = d3.select("#bar_"+level1Id).append("svg").attr("width",650)
	...
	if(datas.length == 0){
		...
		var text = svg.append("svg:text").attr("class", "nobar").attr("x", 450).attr("y", 190).attr("dy", ".35em").attr("text-anchor", "middle").style("font", "100 20px sans-serif").text("No Data");
		var bbox = text.node().getBBox();
	}
