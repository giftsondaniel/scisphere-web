	...
	if(minvall < 0){
		...
		if(( QrMaxValue < maxvall) && ( maxvall_QrMaxValue_2 < maxvall)){//maxvall*(2.1/4)
			...
			svg.append("g").attr("transform", "translate(240,0)")
			  		 .call(d3.svg.axis().scale(lower).orient("left").ticks(6)).style({"font": "Roboto, sans-serif",'font-size':'9px',"fill":"#8A8A8A"});

			//UPPER Bar chart	
			//svg.append("g").attr("transform", "translate(240,0)")
			  	//	 .call(d3.svg.axis().scale(upper).orient("left").ticks(1)).style({"font": "Roboto, sans-serif",'font-size':'9px',"fill":"#8A8A8A"});	
			//svg.append("g").attr("transform", "translate(240,0)").call(d3.svg.axis().scale(upper).orient("left").ticks(1).tickValues([ maxvall])).style({"font": "Roboto, sans-serif",'font-size':'10px',"fill":"#5F5F5F"});	
			...
			//UPPER Bar chart	
			//svg.append("g").attr("transform", "translate(240,0)").call(d3.svg.axis().scale(upper).orient("left").ticks(1))
