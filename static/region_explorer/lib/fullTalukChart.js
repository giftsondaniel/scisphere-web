function level_3_LevelChart( fld, yr, levelType,lvl2_id ){
	...
	if(minvall < 0){
		...
		if(( QrMaxValue < maxvall) && ( maxvall_QrMaxValue_2 < maxvall)){//maxvall*(2.1/4)
			...
			svg.append("g").attr("transform", "translate(240,0)").call(d3.svg.axis().scale(upper).orient("left").ticks(1).tickValues([maxvall ]).tickFormat(function(d) { return formatValue(d); /* return valueRound(maxvall ,numberFormat);*/ })).style({"font": "Roboto, sans-serif",'font-size':'10px',"fill":"#5F5F5F"});	
