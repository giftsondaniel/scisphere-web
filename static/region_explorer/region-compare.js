	...
	if(Object.keys(prms).length > 0){
		...
		if(levelId==3){
			for(var i=0;i<level1Arr.length;i++){
				...
				var level2KeyArr=prms[keyname2].split(",");

				for(var j=0;j<level2KeyArr.length;j++){
					var level2value = level2KeyArr[j];
					...
					for(var k=0;k<level3Arr.length;k++){
						...
						document.getElementById("geo_level2_"+jVal).value=level2KeyArr[j];
						$("#geo_level2_"+jVal).data("selectBox-selectBoxIt").add();
...
function fieldChange(fieldid){
	...
	if(selectedField != fieldId){
		...
		if(geoLevel==2){
			...
			prms["field"]=fieldId.toUpperCase();
...
function applyGeoVariable(){
	...
	if(geoLevel==2){
		...
		prms["field"]=fieldId;
		prms["level1"]=stateArr.toString();
		prms["geoid"]=selectedGeoId.sort().toString();
		prms["level"]=2;
	}else if(geoLevel==3){
		var stateArr=[];
		for(var key in selectedGeoVariable){
...
function removeSVG(){
	...
	//d3.select("#map_legend").select('div').remove();
...
function dataLoad(level0_Id,fieldId,yearId,levelId){
	...
	if(levelId){
		...
		d3.json("/app/places/compare?id="+selectedGeoId+"&category="+category+"&varkey="+fieldId.toLowerCase()+"&index="+index_name+"&level="+parseInt(levelId)+"", function(data) {
...
function compareData_level2(d){
	...
	function loadTopoJson(level1Id){
		chart(fieldId.toLowerCase(),yearId,400,500,level1Id);
		var tn;
...
function compareData_level3(d){
	...
	// Filtering by field and year
	key = l3_KeyVal.filter(fieldId.toLowerCase());
