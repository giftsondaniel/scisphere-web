	...
	for(var i=0;i<rankCategory.length;i++){
		for(var key in data['rank']){
			if(key == rankCategory[i]){
				...
				var snapshotValue=[];
				var categories=[];
				var rankKeys = _.omit(data.rank[key], ['id_0','id_1','id_2','id_3','id_4','name_0','name_1','name_2','name_3','name_4','collecteddate','geolocation','key','latitude','longitude','leveltype','docid','org_id',"uploaddate","District","type","tempid"]);
...
function snapshotCustomLoad(data){
	...
	for(var j=0;j<selectedVariable.length;j++) {
		for(var key in data['rank']){
			var rankKeys = _.omit(data.rank[key], ['id_0','id_1','id_2','id_3','id_4','name_0','name_1','name_2','name_3','name_4','collecteddate','geolocation','key','latitude','longitude','leveltype']);
			for(var key1 in rankKeys){
...
function snapshotTop10Load(data){
	...
	var rankKeys = _.omit(data['rank']['economic'], ['id_0','id_1','id_2','id_3','id_4','name_0','name_1','name_2','name_3','name_4','collecteddate','geolocation','key','latitude','longitude','leveltype']);
	for(var key1 in rankKeys){
		if(rankKeys[key1] == 0 || rankKeys[key1]){
			var obj={};
...
function snapshotChartLoad(data,categories,dollars,snapshotValue,canvasname,svgname,rankCategoryId){
	...
	var yAxis = d3.svg.axis();
		...
		.scale(yscale)
		.tickSize(3)
		.tickFormat(function(d,i){ return categories[i].trim(); })
	...
	if(!data.rank[rankCategoryId]){
		return false;
	}


	var level2Name = data.rank[rankCategoryId].name_2;
	var level1Value = document.getElementById('level_1_Id');
 	var level1Name = level1Value.options[level1Value.selectedIndex].innerHTML;
	var textValue = level2Name+" - Percentile among in "+level1Name;
	if(data.rank[rankCategoryId].name_4){
		var level4Name = data.rank[rankCategoryId].name_4;
		var level3Name = data.rank[rankCategoryId].name_3;
		textValue = level4Name.toTitleCase()+','+ level3Name.toTitleCase()+" - Percentile among all villages in "+level1Name.toTitleCase();
	} else if(data.rank[rankCategoryId].name_3){
		var level3Name = data.rank[rankCategoryId].name_3;
