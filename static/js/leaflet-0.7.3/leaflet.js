...
!function (t, e, i) {
	...
	o.Mixin.Events = {
		addEventListener : function (t, e, i) {
			...
			h = t[n],
			p ? (l = h + "_idx", u = l + "_len", c = d[l] = d[l] || {}, c[p] || (c[p] = [], d[u] = (d[u] || 0) + 1), c[p].push(r)) : (d[h] = d[h] || [], d[h].push(r));
			return this
		},
	...
	o.DomUtil = {
		...
		setPosition : function (t, e, i) {
			t._leaflet_pos = e,
			!i && o.Browser.any3d ? t.style[o.DomUtil.TRANSFORM] = o.DomUtil.getTranslateString(e) : (t.style.left = e.x + "px", t.style.top = e.y + "px")
		},
	...
	o.Popup = o.Class.extend({
			...
			_updateContent : function () {
				...
				//Marker popup//Added
				var latlong = this._latlng.lat.toString().replace('.','_')+'_'+this._latlng.lng.toString().replace('.','_');

				if(document.getElementById('wb_data_id1_'+latlong)){
					document.getElementById('wb_data_id1_'+latlong).value = workbookKeys[0];
					$('#wb_data_id1_'+latlong).attr('data-tooltip','list-item-'+workbookKeys[0]+'');
					$('#wb_data_id1_'+latlong).attr('data-tooltip2','st-arrow');
					$('#wb_data_id1_'+latlong).attr('data-target','#range-'+workbookKeys[0]+'');

					document.getElementById('wb_data_id2_'+latlong).value = workbookKeys[1];
					$('#wb_data_id2_'+latlong).attr('data-tooltip','list-item-'+workbookKeys[1]+'');
					$('#wb_data_id2_'+latlong).attr('data-tooltip2','st-arrow');
					$('#wb_data_id2_'+latlong).attr('data-target','#range-'+workbookKeys[1]+'');

					document.getElementById('wb_data_id3_'+latlong).value = workbookKeys[2];
					$('#wb_data_id3_'+latlong).attr('data-tooltip','list-item-'+workbookKeys[2]+'');
					$('#wb_data_id3_'+latlong).attr('data-tooltip2','st-arrow');
					$('#wb_data_id3_'+latlong).attr('data-target','#range-'+workbookKeys[2]+'');

					document.getElementById('wb_data_id4_'+latlong).value = workbookKeys[3];
					$('#wb_data_id4_'+latlong).attr('data-tooltip','list-item-'+workbookKeys[3]+'');
					$('#wb_data_id4_'+latlong).attr('data-tooltip2','st-arrow');
					$('#wb_data_id4_'+latlong).attr('data-target','#range-'+workbookKeys[3]+'');
				}

				$('#display1').bind('click', function() {
				    var $divs = $('.divs');
					...
					/*if(offset == 1){
						$divs.hide().slice(divsCount-1, divsCount ).show();
						offset = divsCount;
						offset -= 1;
						console.log(offset);
						if (offset <= 1) offset = divsCount ;//clusterMarkerNo;	
						offst =offset; 
					}*/
					 if(offset > 0) {
						$divs.hide().slice(offset-2, offset-1 ).show();
						offset -= 1;						//console.log(offset);
						if (offset < 1) offset = divsCount ;//clusterMarkerNo;	
						offst =offset; 
					}
				        document.getElementById("page").innerHTML = offset;
					if(offset == 1){  document.getElementById("display1").disabled = true; 	}
					else{ document.getElementById("display1").disabled = false;  }

					if(offset == divsCount){  document.getElementById("display").disabled = true; 	}
					   else{ document.getElementById("display").disabled = false;  }
				...
				$('#display').bind('click', function() {
					...
					//console.log(offset);
					offset = offst;
					$divs.hide().slice(offset, offset + 1).show();
					offset += 1;
					//console.log(offset);
					offst =offset;
					//console.log(offset);
	...
	o.Map.include({
		...
		closePopup : function (t) {originWithoutLaunchClosePopup();	//added elango
