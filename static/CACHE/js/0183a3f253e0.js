...
document.getElementById("next-page-pin").onclick=function(){$pin_table_rows=$('.pins-load div.pin');if(pinTotalPageLength>=offset)loadPins();}
document.getElementById("last-page-pin").onclick=function(){$pin_table_rows=$('.pins-load div.pin');if(pinTotalPageLength>=offset)loadPins();}
...
var template=Handlebars.compile($('#pins-template').html());var html=template({pins:pins.objects});console.log(html);$('#pins').append(html);tileLayout();lightbox();$('#pins').ajaxStop(function(){$('img').load(function(){$(this).fadeIn(300);});});if(pins.objects.length<apiLimitPerPage){}else{$(window).scroll(scrollHandler);}
pinPagination(pinTotalPageLength);});offset+=apiLimitPerPage;}
var pinTotalPageLength=0;var offset=0;loadPins();});function pinPagination(pinTotalPageLength){$pin_table_rows=$('.pins-load div.pin');var pin_row_limit=4;pinTotalPage=Math.ceil(pinTotalPageLength/pin_row_limit);$('#pg1').jqPagination('option','max_page',pinTotalPage);}
function pinPaginationGoTo(){var current_page=document.getElementById('pg-pin-goto').value;$pin_table_rows=$('.pins-load div.pin');var pin_row_limit=4;pinTotalPage=Math.ceil($pin_table_rows.length/pin_row_limit);$('#pg1').jqPagination('option','max_page',pinTotalPage);if(pinTotalPage>=current_page){$('#pg1').jqPagination('option','max_page',pinTotalPage);$('#pg1').jqPagination('option','current_page',current_page);}else{$('#pg1').jqPagination('option','max_page',pinTotalPage);$('#pg1').jqPagination('option','current_page',pinTotalPage);}}
$(document).ready(function(){$pin_table_rows=$('.pins-load div.pin');var pin_row_limit=4;pinTotalPage=Math.ceil($pin_table_rows.length/pin_row_limit);var pin_page_table=function(page){var offset=(page-1)*pin_row_limit,limit=page*pin_row_limit;$pin_table_rows.hide();$pin_table_rows.slice(offset,limit).show();}
$('.pagination').jqPagination({link_string:'/?page={page_number}',max_page:pinTotalPage,paged:pin_page_table});});$(window).load(function(){var uploadedImage=false;var editedPin=null;function getFormData(){return{submitter:currentUser,url:$('#pin-form-image-url').val(),description:$('#pin-form-description').val(),tags:cleanTags($('#pin-form-tags').val())}}
