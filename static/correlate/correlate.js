...
function loadCorrelateView(){
                	...
                	'<h2 class="page-title" id="fields-title_'+selectedVariable[i]+'"> Loading Data ...</h2>'+
                        ...
                        '<div class="tab-content correlate-tab-content clearfix">'+
                             '<div class="tab-pane active" id="scatter_'+selectedVariable[i]+'">'+
                            	'<div class="col-lg-12 correlate_'+selectedVariable[i]+'">'+
	...
	for(var i=0;i<selectedMyVariable.length;i++){
			...
			viewContent+='<div class="col-lg-12" id="correlate_'+selectedMyVariable[i]+'" style="display:none">'+
				'<h2 class="page-title" id="fields-title_'+selectedMyVariable[i]+'"> Loading Data ...</h2>'+
			    	'<div class="tab-sec correlate-tab clearfix">'+
			            '<ul class="nav nav-tabs rightpanel-map-tab" role="tablist" >'+
			                '<li class="active"><a href="#scatter_'+selectedMyVariable[i]+'" role="tab" data-toggle="tab" id="tab-map">Scatter</a></li>'+
			                '<li><a href="#whiskers_'+selectedMyVariable[i]+'" role="tab" data-toggle="tab" id="tab-grid">Box &amp; Whiskers</a></li>'+
			            '</ul>'+
			        '</div>'+
			        '<div class="tab-content correlate-tab-content clearfix">'+
			             '<div class="tab-pane active" id="scatter_'+selectedMyVariable[i]+'">'+
			            	'<div class="col-lg-12 correlate_'+selectedMyVariable[i]+'">'+
			                	'<div class="col-lg-7 tab-left">'+
			                    	'<p class="title-link-sec"><a onclick="change_axis('+"'"+selectedMyVariable[i]+"'"+');" title="Transpose Axes" style="color:red;" id="transpose_'+selectedMyVariable[i]+'" class="title-link">Transpose Axes</a></p>'+
			                        '<div class="graph-wrap" id="Scatter_chart_'+selectedMyVariable[i]+'" >'+
							'<div> Loading Data ... </div>	'+
			                        '</div>'+
			                    '</div>'+
			                    '<div class="col-lg-5">'+
			                    	'<div class="corr-block-view">'+
			                        	'<table border="0" class="block-view-table">'+
			                            	'<tr>'+
			                                	'<td width="110"><h4 class="title">Correlation</h4></td>'+
				                                '<td><h3 class="value" id="cor_val_'+selectedMyVariable[i]+'"><sup><a href="#" class="correlate-info" data-toggle="tooltip" data-placement="right" title="Info"></a></sup></h3></td>'+
			                                '</tr>'+
			                                '<tr>'+
			                                	'<td><h4 class="title">R<sup>2</sup> of linear fit</h4></td>'+
			                                   	'<td><h3 class="value" id="r2_val_'+selectedMyVariable[i]+'"><sup><a href="#" class="correlate-info" data-toggle="tooltip" data-placement="right" title="Info"></a></sup></h3></td>'+
			                                '</tr>'+
			                            '</table>'+
			                        '</div>'+
			                    '</div>'+
			                '</div>'+
			            '</div>'+
			            '<div class="tab-pane" id="whiskers_'+selectedMyVariable[i]+'">'+
			            	'<div class="col-lg-12 correlate_'+selectedMyVariable[i]+'">'+
			                	'<div class="col-lg-7 tab-left">'+
			                    	'<p class="title-link-sec"><a onclick="change_axis();" title="Transpose Axes" class="title-link-dis">Transpose Axes</a></p>'+
			                        '<div class="graph-wrap"  >'+
	...
	if(correlateData){
		if(correlateData.correlatedata){
			for(key in correlateData.correlatedata){
				if(( $.inArray(key, selectedVariable) != -1)){
				...
				if(( $.inArray(key, selectedMyVariable) != -1)){
					d3.select('#correlate_'+key).style('display','');	
					document.getElementById('Scatter_chart_'+key).innerHTML="";
					document.getElementById('cor_val_'+key).innerHTML = "";
					document.getElementById('r2_val_'+key).innerHTML = "";
					$('#Scatter_chart_'+key).append('<img src="/media/correlate/'+key+'_1_'+correlateData.id+'.png"></img>');
					document.getElementById('cor_val_'+key).innerHTML = numberFormat(correlateData.correlatedata[key]["corcoef"]);
					document.getElementById('r2_val_'+key).innerHTML = numberFormat(correlateData.correlatedata[key]["rsquare"]);
				}
			}
		}
	}
...
function dataLoad(level0_Id,level1_Id,fieldid,yearval){
	...
	for(var i=0;i<selectedCorrelationFields.length;i++){
		// added
		d3.select('.col-lg-12').style('display','none');
		// added
		if(myVariablesKey){
			if(myVariablesKey[selectedCorrelationFields[i]]){
	...
	//added
	if(myvarKeyId.length==2){
		var fieldTitle = myvarKeyName[0] +' <span class="page-subtitle">vs</span> '+ myvarKeyName[1];
		if(document.getElementById("fields-title_"+myvarKeyId[0])){
			document.getElementById("fields-title_"+myvarKeyId[0]).innerHTML = fieldTitle;
		}else{
			document.getElementById("fields-title_"+myvarKeyId[1]).innerHTML = fieldTitle;
		}
	}
	//added
	...
	$.ajax({
	    ...
	    success: function (data) {
			...
			for(key in data.correlatedata){
				...
				if(( $.inArray(key.toUpperCase(), selectedVariable) != -1)){
					imageUniqueId=data.id;
					d3.select('#correlate_'+key.toUpperCase()).style('display','block');
					d3.select('.correlate_'+key.toUpperCase()).style('display','block');	
					document.getElementById('Scatter_chart_'+key.toUpperCase()).innerHTML="";
					document.getElementById('cor_val_'+key.toUpperCase()).innerHTML = "";
					document.getElementById('r2_val_'+key.toUpperCase()).innerHTML = "";
				...
				if(( $.inArray(key, selectedMyVariable) != -1)){
					imageUniqueId=data.id;
					d3.select('#correlate_'+key).style('display','block');	
					d3.select('.correlate_'+key).style('display','block');	
					document.getElementById('Scatter_chart_'+key).innerHTML="";
					document.getElementById('cor_val_'+key).innerHTML = "";
					document.getElementById('r2_val_'+key).innerHTML = "";
					$('#Scatter_chart_'+key).append('<img src="/media/correlate/'+key+'_1_'+data.id+'.png"></img>');
					document.getElementById('cor_val_'+key).innerHTML = numberFormat(data.correlatedata[key]["corcoef"]);
					document.getElementById('r2_val_'+key).innerHTML = numberFormat(data.correlatedata[key]["rsquare"]);
				}
