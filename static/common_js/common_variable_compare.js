...
function loadVariableCategory(){
	...
	var categoryKeys= [];
	for(var i=0;i<variableCategory.length; i++){
		categoryKeys.push(variableCategory[i]._source['category']);/*.replace(/\w\S* /g, function(txt){return txt.charAt(0).toUpperCase() + txt.substr(1).toLowerCase();})*/
	...
	if(selectedMyVariable){
		...
		for(var i=0;i<selectedMyVariable.length;i++){
			selectedMyVariableContent +='<tr>'+
				    '<td width="20px">'+sNo+'</td>'+
				    '<td>'+myVariablesKey[selectedMyVariable[i]]+'</td>'+
...
function applySelectedVariable(){
	...
	if(selectedVariable.length<10){
	    contextualVariableContent+='<li class="panel"><a data-toggle="modal" data-target=".left-panel-popup" onclick="contextClick();">Add more variables...</a></li>';
        }
	...
	if(selectedMyVariable.length<10){
	    ...
	    if(sub_list['plus_plans'].length === 0){
		    ...
		    myVariableContent+='<li class="panel"><a data-toggle="modal" data-target=".left-panel-popup" onclick="myVarClick();">Add more variables...</a></li>';
	    }
       }
	...
	/*if(document.getElementById('levelId')){	
		...
		mysphere_url_list="/app/mysphere_mybuilds_for_region_snapshot/"+level1_Id+"?level0="+level0_Id+"&level="+levelval+"&region_key="+fieldid+"&selectedvariables="+JSON.stringify(selectedVariable)+"&selectedMyVariables="+JSON.stringify(selectedMyVariable)+"";
		...
		if(Object.keys(prms).length > 0){
		    ...
		    mysphere_url_list="/app/mysphere_mybuilds_for_snapshot/"+level1Id+"?level0="+level0Id+"&levels="+levels+"&levelid="+levelId+"&selectedvariables="+JSON.stringify(selectedVariable)+"&selectedMyVariables="+JSON.stringify(selectedMyVariable)+"";
		    d3.json(mysphere_url_list,function(data){
			var str = JSON.stringify(data.MyBuild[0])
			var newvar = JSON.parse(str);
