	...
	if(booleanMasterKey.length>0){
		for(var i=0;i<booleanMasterKey.length;i++){
			...
			'<div id="range-'+booleanMasterKey[i].key+'" class="collapse in range-sec">'+
			  '<p class="range-val" id="'+booleanMasterKey[i].key+'-val">'+
				'<label for="id-'+booleanMasterKey[i].key+'1" style="cursor: pointer;">Yes &nbsp;</label>'+
				'<input type="radio" id="id-'+booleanMasterKey[i].key+'1" name="'+booleanMasterKey[i].key+'" value="'+booleanMasterKey[i].maxval+'" />'+
				'<label for="id-'+booleanMasterKey[i].key+'2" style="cursor: pointer;">&nbsp; &nbsp; No &nbsp;</label>'+
				'<input type="radio" id="id-'+booleanMasterKey[i].key+'2" name="'+booleanMasterKey[i].key+'" value="'+booleanMasterKey[i].minval+'" />'+
			  '</p>'+
			'</div>'+
		      '</li>';*/
		      s+='<li class="panel">'+
...
function onViewData(){
	...
	if(booleanMasterKey.length>0){
		for(var i=0;i<booleanMasterKey.length;i++){
			/*if (document.getElementById("id-"+booleanMasterKey[i].key+"1").checked) {
			  banked = document.getElementById("id-"+booleanMasterKey[i].key+"1").value;

			}else if (document.getElementById("id-"+booleanMasterKey[i].key+"2").checked) {
			  banked = document.getElementById("id-"+booleanMasterKey[i].key+"2").value;
			}*/
...
function onClickMarker(e){
	...
	for(var i=0;i<selectedVariable.length;i++){
		if($("#"+selectedVariable[i]+"_collapse").hasClass("") == true){
			var value = _.where(jsonData, {latitude:e.latlng.lat.toString(),longitude:e.latlng.lng.toString()});
			var keyName = variablesKey[selectedVariable[i].toLowerCase()];

			if(labelKeyValue[keyName]){
				labelKeyValue[keyName]=labelKeyValue[keyName] + value[0][keyName];
			}else{
				labelKeyValue[keyName]=value[0][keyName];
			}
		}
	}
	var latitude=e.latlng.lat.toString(),longitude=e.latlng.lng.toString();

	$(".c_"+latitude.replace(".","_")+"_"+longitude.replace(".","_")).css('color','red');

	var html="";
	for(var key in labelKeyValue){
		html+='<br><label>'+key+' : </label><label>'+labelKeyValue[key]+'</label>';
	}
	valInfo.update(html);


	if(distanceLatLongArr.length<2){
		distanceLatLongArr.push(e.latlng);
	}

	if(distanceLatLongArr.length>1){

		ployLine.removeLayer(firstpolyline);
		map.removeLayer(ployLine);

		firstpolyline = new L.Polyline(distanceLatLongArr, {
			color: 'red',
			weight: 3,
			opacity: 0.5,
			smoothFactor: 1
		...
		var distanceKM="";
		$.each(distanceLatLongArr, function(i, latlng){
		if(tempLatLng == null){
		    tempLatLng = latlng;
		    return;
		}
...
markers = new L.FeatureGroup();/*Layer group created*/
		...
		for(var i=0;i<jsonData.length;i++){
			if(jsonData[i].latitude){
				if(jsonData[i].longitude){
					...
					marker = new L.marker([jsonData[i].latitude,(jsonData[i].longitude)], {icon: jobMarkerIcon, title:jsonData[i][""+levelType+""],type:"Place"}).on('click', onClickMarker);
