...
function editWorkbook(){
	if(originLatLngArr.length>0){
		...
		if(distanceLatLongArr.length>0 && polyLineArr.length>0){
...
function onClickMarker(e){
	...
	// Edit origin marker
	if(originLatLngArr.length>0){
		if(originLatLngArr[0]['lat'] == latitude && originLatLngArr[0]['lng'] == longitude){
			originLatLng=true;
			setTimeout(function(){
	...
	var distanceRepeat = _.where(distanceLatLongArr, {lat:latitude,lng:longitude});
	...
	if(distanceRepeat.length>0){
		var myArray = distanceLatLongArr.filter(function( obj ) {
			return obj.lat !== latitude && obj.lng !== longitude;
		});
			...
			for(var i=0;i<jsonData.length;i++){
				if(jsonData[i].latitude){
					if(jsonData[i].longitude){
						...
						var html ='';
						html +='<div><input type="checkbox" id="idorigin_'+lat.replace('.','_')+"_"+lng.replace('.','_')+'" onclick="originClick('+lat+','+lng+');">'+
