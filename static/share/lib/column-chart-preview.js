function shareColumnChart() {
  ...
  var margin = {top: 50, right: 80, bottom: 50, left: 50},
      width = widthShare,//550,
      height = widthShare,//350,
      xRoundBands = 0.2,
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      if(widthShare == 450 ){ xAxisLengthMinus = (130*1);}
      if(widthShare == 350 ){ xAxisLengthMinus = (130*(0.78));}	
      if(widthShare == 250 ){ xAxisLengthMinus = (130*(0.56));}	
      ...
      var svgShare = d3.select(this).selectAll("#mapShare").data([data]);



      // Otherwise, create the skeletal chart.
      var gEnter = svgShare.enter().append("svg").attr('id','share_svg').append("g");
      gEnter.append("g").attr("class", "bars");
      gEnter.append("g").attr("class", "y axis");
      gEnter.append("g").attr("class", "x axis zero");

      // Update the outer dimensions.
      svgShare .attr("width", width)
          .attr("height", height);

      // Update the inner dimensions.
      var g = svgShare.select("g")
          .attr("transform", "translate(" + margin.left + "," + margin.top + ")");

      // Update the bars.
      var bar = svgShare.select(".bars").selectAll(".bar1").data(data);
