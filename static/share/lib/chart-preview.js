function shareChart(level0Id, level1Id, levelid, data_val ,yr,levelType,minvalue, secval, midval1, forval, maxvall, svgid, fld){
	...
	color = d3.scale.linear().range(shareColors).domain([minvalue, secval, midval1, forval, maxvall]);
	...
	if(fld){
	    if((fld == 'IND_ROAD_01') || (fld == 'IND_ROAD_02') || (fld == 'IND_ROAD_03') ||(fld == 'IND_ROAD_04')){	
		    var svgShare = d3.select("#barShare").append("svg").attr("width",650)
			.attr("height", 450).attr('id','bar1_'+fld).append("g").attr("id", "barChart")
			.attr("transform", "translate(-150,0)");

		    var text = svgShare.append("svg:text").attr("class", "nobar").attr("x", 450).attr("y", 150).attr("dy", ".35em")
	...
	if(minvall < 0){ //Chart with Negative Values come under this Chart.. With Negative bar:
		...
		if((QrMaxValue < maxvall) && (QrMaxValue != 0) ){
			...
			if(minvalue== second_middle_value){
					...
					color = d3.scale.linear().range(shareColors).domain([minvalue, second_second_value, second_middle_value, quartilesVal[2],maxvall ]);	
			}
			...
			//LOWER Bar chart	
			svgShare.append("g").attr("transform", "translate(240,0)").attr("class", "tickValues")
			...
			//UPPER Bar chart	
			svgShare.append("g").attr("transform", "translate(240,0)").attr("class", "tickValues").call(d3.svg.axis().scale(upper).orient("left").ticks(1).tickValues([maxvall ]).tickFormat(function(d) { /* return valueRound(maxvall ,numberFormat);*/ return formatValue(d); })).style({"font": "Roboto, sans-serif",'font-size':'10px',"fill":"#5F5F5F"});
	...
	if(datas.length == 0){
		var svgShare = d3.select("#mapShare").append("svg")
			.attr("width",widthShare)
			.attr("height", widthShare)
			.attr('id','share_svg')
			.append("g")
		...
		var text = svgShare.append("svg:text").attr("class", "nobar").attr("x", 450).attr("y", 190).attr("dy", ".35em").attr("text-anchor", "middle").style("font", "100 20px sans-serif").text("No Data at "+ levelType_ND +" level");
