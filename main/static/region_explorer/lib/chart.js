	...
	if(minvall === maxvall && maxvall === 0){
	 ...
	 if(minvall < 0){			//Chart with Negative Values come under this Chart.. With Negative bar:
		...
		svg.selectAll("rect.upper")
		   ...
		   //UPPER Bar chart	
		   svg.append("g").attr("transform", "translate(230,0)").call(d3.svg.axis().scale(upper).orient("left").ticks(1)).style({"font": "Roboto, sans-serif",'font-size':'10px',"fill":"#5F5F5F"});	
		...
		d3.select("#bar")
			...
			if((QrMaxValue < maxvall) && (QrMaxValue != 0) ){
				...
				//UPPER Bar chart	
				svg.append("g").attr("transform", "translate(240,0)")
			  		 .call(d3.svg.axis().scale(upper).orient("left").ticks(1)).style({"font": "Roboto, sans-serif",'font-size':'10px',"fill":"#5F5F5F"});	
