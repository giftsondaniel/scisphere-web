function level_3_LevelChart_2( fld, yr,chartWidth,chartHeight){
...
var columnBarChartWidth;
	...
	key = l3_KeyVal_2.filter(fld);
	key2 = l3_YearVal_2.filter(yr);
	...
	var data = datas.sort(function(a,b) {
		...
		d3.select("#bar")
		    .datum(barData)
		    .call(columnChart_2(columnBarChartWidth)
		    .width(chartWidth)
		    .height(450)
			...
			if(mid_com){
				...
				var svg = d3.select("#bar").append("svg").style("margin-left", 410).attr("width",chartWidth).attr("height", 450).attr('id','bar1').append("g").attr("id", "barChart2")
