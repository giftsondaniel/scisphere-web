...
var columnBarChartWidth;
	...
	key = keyVal_2.filter(fld);
	key2 = yearVal_2.filter(yr);
	...
	var data = datas.sort(function(a,b) {
	 ...
	 if(minvall < 0){			//Chart with Negative Values come under this Chart.. With Negative bar:
		...
		var svg = d3.select("#bar").append("svg").style("padding-left", 410).attr("width",chartWidth).attr("height", 450).attr('id','bar2').append("g").attr("id", "barChart2")
		...
		svg.selectAll("rect.upper")
		   ...
		   //SVG Declaring with id named BAR , height , width and Transform
		   svg = d3.select("#bar").append("svg").style("padding-left", 410).attr("width",chartWidth/3).attr("height", 450).attr('id','bar2').append("g").attr("id", "barChart2")
		...
		d3.select("#bar")
			...
			if((QrMaxValue < maxvall) && (QrMaxValue != 0) ){
				...
				var svg = d3.select("#bar").append("svg").style("margin-left", 410).attr("width",chartWidth).attr("height", 450).attr('id','bar2').append("g").attr("id", "barChart2")
				...
				//SVG Declaring with id named BAR , height , width and Transform
				var svg = d3.select("#bar").append("svg").attr("width",chartWidth).attr("height", 450).attr('id','bar2').append("g").attr("id", "barChart")
	...
	if(datas.length == 0){
		var svg = d3.select("#bar").append("svg").attr("width",650)
			.attr("height", 450).attr('id','bar2').append("g").attr("id", "barChart")
