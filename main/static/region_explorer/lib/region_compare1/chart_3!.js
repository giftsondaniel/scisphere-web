//function chart_2(fld, yr,chartWidth,chartHeight){
function chart_3(fld, yr,chartWidth,chartHeight){

var barWidth = 0;
var barChartWidth = 0;
var columnBarChartWidth = 0;
	...
	key = keyVal_3.filter(fld);
	key2 = yearVal_3.filter(yr);
	...
	//var hist = [-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10,-10];
	//histogramLoad(hist);
	...
	var data = datas.sort(function(a,b) {
				...
				if(a.value.trim() =="inf" && b.value.trim() =="inf" ){
					return 0; 
				}
				if(a.value.trim() =="inf"){
				...
				else { return b.value.trim() - a.value.trim();  }				 
				 });
	 ...
	 if(minvall < 0){			//Chart with Negative Values come under this Chart.. With Negative bar:
	    ...
	    if( ( QrMaxValue < maxvall) && ( maxvall_QrMaxValue_2 < maxvall)){
				...
				console.log("ljlkjlkjlkjlkjlkjlkj::::::::::::::::::::!  c33333");
		...
		var svg = d3.select("#bar").append("svg").attr("width",chartWidth).attr("height", 450).attr('id','bar3').append("g")
				.attr("id", "barChart3").attr("transform", "translate(640,10)");
		...
		svg.selectAll("rect.upper")
		   ...
		   //LOWER Bar chart	
		   svg.append("g").attr("transform", "translate(230,0)").call(d3.svg.axis().scale(lower).orient("left").ticks(6));

		   //UPPER Bar chart	
		   svg.append("g").attr("transform", "translate(230,0)").call(d3.svg.axis().scale(upper).orient("left").ticks(2));	
		   ...
		   svg = d3.select("#bar").append("svg").attr("width",chartWidth).attr("height", 450).attr('id','bar3').append("g").attr("id", "barChart3")
		...
		for( i = 0; i < datas.length; i++) {
		    d = [];		 
		    d["0"] = datas[i].name_2+"_"+datas[i].id_3;
		    d["1"] = +datas[i].value;
		    barData.push(d);
		}
		d3.select("#bar")
			...
			if((QrMaxValue < maxvall) && (QrMaxValue != 0) ){
				...
				var svg = d3.select("#bar").append("svg").style("margin-left", 810).attr("width",chartWidth).attr("height", 450).attr('id','bar3').append("g").attr("id", "barChart3")
				...
				//SVG Declaring with id named BAR , height , width and Transform
				var svg = d3.select("#bar").append("svg").attr("width",chartWidth).attr("height", 450).attr('id','bar3').append("g").attr("id", "barChart3")
	...
	if(datas.length == 0){
		var svg = d3.select("#bar").append("svg").attr("width",650)
			.attr("height", 450).attr('id','bar3').append("g").attr("id", "barChart3")
			.attr("transform", "translate(640,10)");
