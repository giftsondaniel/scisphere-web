...
function columnChart(columnBarChartWidth) {
...
var yrtype = yearType.options[yearType.selectedIndex].value;
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      // Update the y-scale.
      yScale
          .domain(d3.extent(barDataCompare.map(function(d) { return d[1];} )))
          .range([height - margin.top - margin.bottom, 0])
          .nice();
          

      // Select the svg element, if it exists.
      var svg = d3.select('#bar').selectAll("svg").data([data]);
...
function columnChart_2(columnBarChartWidth) {
  var margin = {top: 20, right: 30, bottom: 50, left: 50},
      width = 420,
      height = 420,
      xRoundBands = 0.2,
...
var yrtype = yearType.options[yearType.selectedIndex].value;
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      // Select the svg element, if it exists.
      var svg = d3.select('#bar2').selectAll("svg").data([data]);
...
function columnChart_3(columnBarChartWidth) {
  var margin = {top: 20, right: 30, bottom: 50, left: 50},
      width = 420,
      height = 420,
      xRoundBands = 0.2,
...
var yrtype = yearType.options[yearType.selectedIndex].value;
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      // Select the svg element, if it exists.
      var svg = d3.select('#bar3').selectAll("svg").data([data]);
