function level_3_LevelChart_3( fld, yr,chartWidth,chartHeight){
...
var columnBarChartWidth;
	...
	key = l3_KeyVal_3.filter(fld);
	key2 = l3_YearVal_3.filter(yr);
	...
	var data = datas.sort(function(a,b) {
				...
				if(a.value.trim() =="nan" && b.value.trim() =="nan" ){
					return 0; 
				}
				if(a.value.trim() =="nan"){
					return b.value - maxvall; 
		...
		d3.select("#bar")
		    .datum(barData)
		    .call(columnChart_3(columnBarChartWidth)
		    .width(chartWidth)
		    .height(450)
			...
			if(mid_com){
				...
				var svg = d3.select("#bar").append("svg").style("margin-left", 810).attr("width",chartWidth).attr("height", 450).attr('id','bar1').append("g").attr("id", "barChart3")
	...
	if(datas.length == 0){
		var svg = d3.select("#bar").append("svg").attr("width",650)
			.attr("height", 450).attr('id','bar1').append("g").attr("id", "barChart3")
