function level_3_LevelChart( fld, yr, levelType,lvl2_id ){
	...
	if(minvall < 0){
		...
		var cnt = 0;
		//if((quartilesVal_3 < maxvall ) &&  (QrMaxValue < maxvall)){//maxvall*(2.1/4)
		if(( QrMaxValue < maxvall) && ( maxvall_QrMaxValue_2 < maxvall)){//maxvall*(2.1/4)
			...
			//UPPER Bar chart	
			svg.append("g").attr("transform", "translate(240,0)")
			  		 .call(d3.svg.axis().scale(upper).orient("left").ticks(2 )).style({"font": "Roboto, sans-serif",'font-size':'9px',"fill":"#8A8A8A"});	

		} else if(QrMaxValue > maxvall){
