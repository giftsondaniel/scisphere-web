function chart(fld, yr,chartWidth,chartHeight){
...
var columnBarChartWidth;
	...
	var data = datas.sort(function(a,b) {
	 ...
	 if(minvall < 0){			//Chart with Negative Values come under this Chart.. With Negative bar:
		...
		var svg = d3.select("#bar").append("svg").attr("width",chartWidth).attr("height", 450).attr('id','bar1').append("g").attr("id", "barChart").attr("transform", "translate(-150,10)");
		...
		svg.selectAll("rect.upper")
		   ...
		   //SVG Declaring with id named BAR , height , width and Transform
		   svg = d3.select("#bar").append("svg").attr("width",chartWidth).attr("height", 450).attr('id','bar1').append("g").attr("id", "barChart")
