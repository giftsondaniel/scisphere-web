...
function snapshotChart(data){
	...
	var canvas = d3.select('#wrapper')
		...
		.attr({'width':500,'height':550,'padding-left':550});
	...
	if(data.rank[0]._source.name_3){
		var level3Name = data.rank[0]._source.name_3;
		textValue = level3Name+', '+level2Name+" - Percentile among in "+level1Name;
	}else{
		textValue = level2Name+" - Percentile among in "+level1Name;
	}
...
function wrap(text, width) {
  text.each(function() {
	...
	tspan.text(line.join(" "));
	line = [word];
	tspan = text.append("tspan").attr("x", -4).attr("y", (++lineNumberChart * 15)).text(word);
     }
    }
  });
}
